...
I think it would be smart to make a single file containing initialization of the DOF so that for all the software we write later, we can just import the DOF initialization so that it cuts down on complexity. Just need to cite this in comments and the location of DOF intiialization core components so that future people won't have trouble.

For failed trials, the only time this should happen is in two scenarios: first is that theres just no established connection, which can be discovered by having a pre-check that validates responses, and will otherwise error out if nothing works. Second scenario is connection loss during testing, which can similarly just error out.

A buffer is probably fine for data collection, where everything is dumped into a single CSV generation when the test ends. Half-baked data isn't useful to us, especially if say for example the DOF loses connection mid-way. There just isn't much we can do with that data, it's all or nothing. So no need to worry about write-as-we-go, should just buffer.

And to start off the program, having a dry-run with a few trials to make sure each profile works is essential for pre-callibration and making sure the DOF is okay and ready to be tested.
